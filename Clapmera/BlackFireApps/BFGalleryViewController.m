...
-(id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil mediaProvider:(BFGAssetsManagerProvider)mediaProvider{
    ...
    if (self) {
        if(!_productsArray){
            _isShowingFullSizeGallery=FALSE;
            self.mediaProvider=mediaProvider;
        }
    }
    return self;
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [self performSelector:@selector(startLoadingContent) withObject:nil afterDelay:1];
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSInteger numberOfImageViewsInCell= [[cell imageViews] count];
    long index0= indexPath.row * numberOfImageViewsInCell;
...
-(void)showFullSizeGalleryWithImageSelected:(UIImageView *)imageView{
    if(_isShowingFullSizeGallery==TRUE)
        return;
    
    if([self.bar isFirstResponder]){
        [self.bar resignFirstResponder];
    }
    _isShowingFullSizeGallery=TRUE;
...
-(void)didSelectImage:(UITapGestureRecognizer *)tap{
    if(self.mediaProvider==BFGAssetsManagerProviderPhotoLibrary){
        ...
        ALAsset * image=_productsArray[index];
        NSString * type= [image valueForProperty:ALAssetPropertyType];
        if([type isEqualToString:ALAssetTypeVideo]){
            NSURL * url=[[image defaultRepresentation] url];
